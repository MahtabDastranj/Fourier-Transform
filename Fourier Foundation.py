...
plt.plot(np.cos(x), np.sin(x), 'ro')        # draw unit circle for refrance
# plot_syntaxs :[
# 	'r--' = 'red dashes';
# 	'bs' = 'blue squares';
# 	'g^' = 'green triangles';
...
# However it's only true for frequencies in step of 1 or 0.5.This is because of the sampling rate
# if we try 4.35 dp is going to be larger.
ampl1 = ampl2 = 2
phas1 = phas2 = np.pi/2
...
plt.title('Signal')
plt.show()
# plotting the dot point between the signal and the sinew for each frequency
...
plt.show()
dps = np.zeros(len(sinefrex), dtype=complex)        # returning a list of 0+0j
for fi in range(len(dps)):
...
plt.show()
# illustration of the effect of phase offsets on dot product
